...
def delete_files(file_entries):
    """
    Löscht Dateien wie die GUI-Funktion delete_selected(),
    aber ohne GUI und ohne Messageboxen.
    file_entries = [(path, size), ...]
    """
    deleted = []
...
def write_log(deleted_entries, log_dir="logs"):
    """
    Schreibt ein Logfile wie die GUI-Methode, aber global.
    """
    try:
